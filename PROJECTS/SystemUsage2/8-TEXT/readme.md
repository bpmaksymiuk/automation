...
Open `10-BUILD/index.html` in any modern browser. With no agent running it starts in **demo mode** - every gauge animates from simulated load so you can see the room come alive. Click **Stoke the furnace** to drive the load up and watch the relief valves trip.
...
```
python3 10-BUILD/agent.py            # serves on http://127.0.0.1:8077
```
...
Options:
```
python3 10-BUILD/agent.py --host 127.0.0.1 --port 8077
```
