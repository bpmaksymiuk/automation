...
- **CHANGED FILES:**
  - `./10-BUILD/index.html` — swapped the desktop and mobile grid order so the boilers sit above the furnaces, matching the updated visual intent; `drawPipes()` now renders explicit DISK and NETWORK destination boxes on the pipe canvas so the steam runs visibly surface-mount to labeled boxes instead of only to edge labels.
- **IMPLEMENTATION CAVEATS:**
  - The agent and snapshot schema were unchanged; this is a presentation-only refinement in the client.
  - The updated pipe boxes keep the existing four flows (WRITE/READ/TX/RX) and the existing data rates; only the destination framing changed.
  - Verified locally in the browser file view: boilers appear above furnaces and the pipe panel shows the new labeled boxes.
- **UC/BR COVERAGE:** UC-001, UC-004, UC-005, UC-006, UC-007; BR-037, BR-038.
- **NOTES:** This follow-up aligns the running build with the revised use-case wording without changing runtime behavior or telemetry contracts.
