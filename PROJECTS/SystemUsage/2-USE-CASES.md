...
- **Layout (v1 revision):** The main view is arranged as two paired furnaces and two paired boilers with a connecting pipe network: (row 1) the **CPU red flame** and the **GPU blue flame** sit side by side as the twin furnaces; (row 2) the **RAM boiler** and the **VRAM boiler** sit side by side, their vessel sizes scaled in proportion to actual capacity (e.g. a 64 GB RAM boiler is visibly larger than an 8 GB VRAM boiler); disk and network read/write flows are drawn as **steam pipes** that run from the furnaces around to a set of labelled inlets (reads / receive) and outlets (writes / transmit). The presentation must read as tidy and orderly.
...
- **VRAM placement (v1 revision):** VRAM usage is presented as its own boiler beside the RAM boiler (no longer only a fill bar inside the GPU panel); the GPU panel itself focuses on utilisation as the blue flame.
...
- NOTES: CPU temperature shown when a sensor is available; otherwise omitted. The CPU flame is red to contrast the blue GPU flame beside it.
...
- NOTES: GPU is optional hardware. VRAM is presented separately as its own boiler (see UC-005). The GPU flame is blue to contrast the red CPU flame.
- RELATED: UC-003, UC-005

## UC-005 : Operator - Monitor RAM and VRAM as twin boilers

- GOAL: See system RAM and GPU VRAM usage side by side as two boilers whose sizes reflect their true capacities.
- STEPS:
...
- NOTES: Water-boiling metaphor is the emotional core. When no GPU is present, the VRAM boiler shows an empty/offline vessel without erroring.
...
- ACCEPTANCE CRITERIA:
  ...
  - AC2: Current disk read and write throughput (bytes/sec) are displayed and rendered as steam-pipe flows — write as an outlet pipe, read as an inlet pipe — running from the furnaces to the disk inlets/outlets.
  - AC3: Fill level reflects used percentage accurately (within rounding) for each filesystem.
- NOTES: Read-only; the dashboard never writes to disks. The disk read/write pipes share the same steam-pipe visual language as the network pipes (see UC-007).
...
- ACCEPTANCE CRITERIA:
  - AC1: Current transmit (TX) and receive (RX) rates are displayed in bytes/sec (human-readable units) and rendered as steam-pipe flows — transmit as an outlet pipe, receive as an inlet pipe.
...
- NOTES: Interface selection when multiple NICs exist. Disk and network pipes share one consistent pipe-network layout (see UC-006).
