# Stage 5 Requirements — The Boiler Room
...
> Sources (implementability research): psutil exposes `cpu_percent(percpu=True)`, `virtual_memory()`, `swap_memory()`, `disk_partitions()`, `disk_usage()`, `disk_io_counters()`, `net_io_counters()`, and `process_iter()` — confirming all required metrics are obtainable on Linux via one library (https://psutil.readthedocs.io/). Server-Sent Events provide a one-way server→client live stream over plain HTTP with automatic reconnection (https://developer.mozilla.org/docs/Web/API/Server-sent_events). The CSS media feature `prefers-reduced-motion` is the standard accessibility hook for animation suppression (https://developer.mozilla.org/docs/Web/CSS/@media/prefers-reduced-motion).
