...
check_contains "$stage_kickoff_file" 'grep -c' "Stage kickoff instructions must provide a JN-ID lookup command to prevent duplicate journal IDs."
check_contains "$stage_kickoff_file" 'quality_check_next_command' "Stage kickoff instructions must require executing quality_check_next_command when set by runner."
...
check_contains "$stage_postcheck" 'POSTCHECK_GATE_LINE_NOT_AT_BOTTOM' "Stage postcheck must enforce gate line position (bottom of artifact)."
